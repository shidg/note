...
sleep 2
tar zxvf libevent-2.0.21-stable.tar.gz && cd libevent-2.0.21-stable && ./configure --prefix=/usr && make && make install || exit 1
echo 'OK,libevent-2.0.21 has  been successfully installed!'

cd ..
echo "Start the installation of libpng..."
sleep 2
tar zxvf libpng-1.6.8.tar.gz && cd libpng-1.6.8 && ./configure --prefix=/usr && make && make install || exit 1
#ln -s /usr/lib/libpng15.so.15.12.0  /usr/lib64/libpng15.so.15
echo 'OK,libpng-1.6.28 has  been successfully installed!'

cd ..
echo "Start the installation of jpeg..."
sleep 2
tar zxvf jpegsrc.v9a.tar.gz && cd jpeg-9a && ./configure --prefix=/usr/local/jpeg --enable-shared --enable-static && make && make install || exit 1
echo 'OK,jpeg-v9a has  been successfully installed!'

cd ..
echo "Start the installation of freetype..."
sleep 2
tar zxvf freetype-2.5.3.tar.gz && cd freetype-2.5.3 && ./configure --prefix=/usr/local/freetype && make && make install || exit 1
echo 'OK,freetype-2.5.3 has  been successfully installed!'

cd ..
echo "Start the installation of gd2..."
sleep 2
tar jxvf libgd-2.1.0.bz2 && cd gd/2.1.0 && ./configure --prefix=/usr/local/gd --with-zlib --with-png=/usr --with-jpeg=/usr/local/jpeg --with-freetype=/usr/local/freetype --with-tiff=/usr/ && make && make install || exit 1
echo 'OK,gd-2.1.0 has  been successfully installed!'
...
echo 'OK,MySQL-5.6.15 has  been successfully installed!'
...
echo  'OK,PHP-5.5.8 has  been successfully installed!'
...
./configure --with-openssl --with-php-config=/usr/local/php/bin/php-config && make && make install || exit 1
...
echo 'OK,nginx-1.6.2 has  been successfully installed!'
...
sleep 2
tar zxvf re2c-0.13.7.5.tar.gz && cd re2c-0.13.7.5 && ./configure && make && make install || exit 1
echo "OK,re2c-0.13.7.5 has  been successfully installed!"

#cd ..
#echo "Start the installation of memcached..."
#sleep 3
#tar zxvf memcached-1.4.17.tar.gz && cd memcached-1.4.17 && ./configure --prefix=/usr/local/memcached --with-libevent && make && make install || exit
...
sleep 2
tar zxvf memcache-3.0.8.tgz && cd memcache-3.0.8 && /usr/local/php/bin/phpize && ./configure --enable-memcache --with-php-config=/usr/local/php/bin/php-config && make && make install || exit 1
...
#sleep 2
#tar zxvf ImageMagick-6.8.8-2.tar.gz && cd ImageMagick-6.8.8-2&& ./configure --prefix=/usr/local/imagemagick && make && make install || exit 1
...
#tar zxvf imagick-3.1.2.tgz && cd imagick-3.1.2 && /usr/local/php/bin/phpize && ./configure --with-imagick=/usr/local/imagemagick --with-php-config=/usr/local/php/bin/php-config && make && make install || exit 1
