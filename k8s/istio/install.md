...
> #### 这里要注意，该脚本的作用其实就是从github获取当前最新版本并下载对应的安装包，可能因为网络问到导致脚本无法执行，可手动翻墙下载

```bash
tar zxvf istio-1.20.2-linux-amd64.tar.gz 
mv istio-1.20.2/bin/istioctl /usr/bin
...
![1706247885857](image/install/1706247885857.png)

```bash
kubectl get svc -n istio-system
```

![1706247980592](image/install/1706247980592.png)

### 使用istio将服务对外暴露

1. 为istio-ingressgateway配置EXTERNAL-IP

   ```bash
   kubectl edit svc -n istio-system istio-ingressgateway
   ```

   ![1706248392200](image/install/1706248392200.png)
2. 创建gateway
   ...
   spec:
     ...
     - match:
       ...
       - destination:
           port:
             number: 80
           host: grafana.prometheus.svc.cluster.local  # 这个地方一定要写完全域名，不然可能出现503 cluster not found
   ```
4. ```bash
